...
const Button = ({ children, variant = "default", size = "default", className = "", onClick, ...props }) => {
  const baseClasses = "inline-flex items-center justify-center rounded-lg font-medium transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 transform hover:scale-105 active:scale-95"
  
  const variants = {
    default: "bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 text-white shadow-lg hover:shadow-xl focus:ring-emerald-500",
    outline: "border-2 border-gray-200 hover:border-emerald-500 text-gray-700 hover:text-emerald-600 bg-white hover:bg-emerald-50",
...
const Input = ({ className = "", ...props }) => {
  return (
    <input
      className={`w-full px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all duration-200 bg-white/50 backdrop-blur-sm ${className}`}
      {...props}
    />
  )
}
    ...
    <div className="relative">
      {children.map((child, index) => 
        index === 0 
          ...
          : isOpen && (
              <div key={index} className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-xl border border-gray-200 py-2 z-50 animate-in slide-in-from-top-2 duration-200">
                {child}
              </div>
            )
      )}
...
const DropdownMenuItem = ({ children, className = "", onClick }) => (
  <button className={`w-full text-left px-4 py-2 text-sm hover:bg-gray-50 transition-colors ${className}`} onClick={onClick}>
    {children}
  </button>
)
          ...
          <div className="bg-gradient-to-r from-emerald-500 to-emerald-600 text-white p-6">
            ...
            <div className="mt-4 flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-emerald-100">{contact.title || 'No title'}</p>
              </div>
            </div>
          </div>
                ...
                <div className="flex items-center p-3 bg-gray-50 rounded-lg">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500">
                      {contact.officePhone ? 'Office Phone' : 'Mobile Phone'}
                    </p>
                  </div>
                </div>
              )}
        ...
        <form onSubmit={handleSubmit} className="h-full flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            <div className="space-y-4">
              <h4 className="font-semibold text-gray-900">Description</h4>
              <textarea
                className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all duration-200 bg-white/50 backdrop-blur-sm"
                rows={4}
...
export default function ContactsPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-emerald-50">
      ...
      {/* Header */}
      <div className="relative bg-white/80 backdrop-blur-lg border-b border-gray-200/50 shadow-sm">
        <div className="px-8 py-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold bg-gradient-to-r from-gray-900 to-emerald-600 bg-clip-text text-transparent">
                Contacts
              </h1>
      ...
      {/* Search and Filters */}
      <div className="relative px-8 py-6 bg-white/60 backdrop-blur-lg border-b border-gray-200/50">
        <div className="flex items-center gap-4">
        ...
        {loading ? (
          <div className="flex justify-center items-center py-20">
            <div className="relative">
              ...
              <div className="absolute inset-0 animate-ping rounded-full h-16 w-16 border-t-4 border-emerald-300 opacity-20"></div>
            </div>
          </div>
        ) : (
          <div className="bg-white/80 backdrop-blur-lg rounded-2xl shadow-xl border border-gray-200/50 overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-gradient-to-r from-gray-50 to-emerald-50 border-b border-gray-200/50">
                  <tr>
                ...
                <tbody className="bg-white/50 backdrop-blur-sm divide-y divide-gray-200/50">
                  {filteredContacts.map((contact, index) => (
                    <tr 
                      key={contact.id} 
                      className="hover:bg-emerald-50/50 transition-all duration-200 hover:shadow-md group"
                      style={{ animationDelay: `${index * 50}ms` }}
                    >
                      <td className="px-8 py-6 whitespace-nowrap">
                        <div className="flex items-center">
                          <div className="relative w-12 h-12 bg-gradient-to-br from-emerald-400 via-emerald-500 to-emerald-600 rounded-xl flex items-center justify-center text-white font-bold text-sm shadow-lg group-hover:shadow-xl transition-all duration-200 group-hover:scale-110">
                      ...
                      <td className="px-6 py-6 whitespace-nowrap">
                        <div className="text-sm text-emerald-600 hover:text-emerald-800 font-medium flex items-center">
                      ...
                      <td className="px-6 py-6 whitespace-nowrap">
                        <div className="text-sm text-gray-900 flex items-center">
                          {(contact.officePhone || contact.mobilePhone) && <Phone className="w-4 h-4 mr-2 text-gray-400" />}
                          {contact.officePhone || contact.mobilePhone || "-"}
                        </div>
                      </td>
                      <td className="px-6 py-6 whitespace-nowrap">
                      ...
                      <td className="px-6 py-6 whitespace-nowrap text-right text-sm font-medium">
                        <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                          <Button
                            ...
                            className="text-emerald-600 hover:text-emerald-800 hover:bg-emerald-50 shadow-sm hover:shadow-md"
                          >
            ...
            {filteredContacts.length === 0 && !loading && (
              <div className="text-center py-20">
                ...
                <div className="text-gray-500 mb-6">Get started by adding your first contact</div>
        ...
        {!loading && filteredContacts.length > 0 && (
          <div className="mt-6 flex items-center justify-between">
            <div className="text-sm text-gray-600 bg-white/60 backdrop-blur-sm px-4 py-2 rounded-full border border-gray-200/50">
              Showing <span className="font-semibold text-emerald-600">{filteredContacts.length}</span> of{" "}
              <span className="font-semibold text-emerald-600">{contacts.length}</span> contacts
            </div>
      ...
      <div className="fixed bottom-6 right-6 md:hidden">
        <Button
          onClick={() => setShowAddContact(true)}
          className="w-14 h-14 rounded-full shadow-2xl hover:shadow-3xl"
        >
